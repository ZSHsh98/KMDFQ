...
class DataLoader(object):
	...
	def __init__(self, dataset, batch_size, imgsize=224, n_threads=4,
	             ten_crop=False, data_path='/home/dataset/cifar', logger=None):
		"""
	...
	def imagenet(self, dataset="imagenet"):
		...
		train_loader = torch.utils.data.DataLoader(
			dsets.ImageFolder(traindir, transforms.Compose([
				transforms.RandomResizedCrop(self.imgsize),
	...
	def dumb_loader(self):
		...
		if self.dataset == "cifar10":
			dataset = dsets.CIFAR10(root=data_root,
			                              train=False,
			                              transform=train_transform,
										  download=True)
		elif self.dataset == "cifar100":
			dataset = dsets.CIFAR100(root=data_root,
			                               train=False,
			                               transform=train_transform,
			                               download=True)
		...
		dumb_loader = torch.utils.data.DataLoader(dataset=dataset,
		                                          batch_size=self.batch_size,
		                                          shuffle=False,
		                                          pin_memory=True,
		                                          num_workers=self.n_threads)
